...
function startNewGame() {
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 201) {
			...
			setTimeout('document.getElementById("error").textContent = ""',
					5000);
...
function listGames() {
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			game_list = JSON.parse(this.responseText)["games"];
...
function login(user) {
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			localStorage.setItem("tic_tac_auth", JSON.stringify({
				"token" : token,
				"user" : user
			}));
...
function validateUser(username, token) {
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			document.getElementById("login").textContent = "You are logged as "
					+ username + ". ";
			document.getElementById("message_link").textContent = "Logout";
			document.getElementById("link").href = "javascript:logout()";
		}
	};
	xhttp.open("GET", BACKEND + "token?token=" + token + "&username="
			+ username, true);
	xhttp.setRequestHeader('Content-type', 'application/json; charset=utf-8');
	xhttp.send();
}
...
function isItRegistered() {
	...
	xhttp.open("GET", BACKEND_GAME_URL + "/" + uuid + "/player?token=" + token,
			true);
	xhttp.setRequestHeader('Content-type', 'application/json; charset=utf-8');
	xhttp.send();
